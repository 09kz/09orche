...
def test_budget_from_env(monkeypatch):
    monkeypatch.setenv("ORCHE_MAX_COST_USD", "5.00")
...
def test_check_budget_raises_once_spent_reaches_limit(monkeypatch):
    monkeypatch.setenv("ORCHE_MAX_COST_USD", "0.05")
    cost.record(0.05)
...
def test_check_budget_allows_spend_under_limit(monkeypatch):
    monkeypatch.setenv("ORCHE_MAX_COST_USD", "1.00")
    cost.record(0.10)
    cost.check_budget("acme/foo")  # should not raise
...
def test_status_without_budget():
    text = cost.status()
    assert "no ORCHE_MAX_COST_USD" in text


def test_status_with_budget(monkeypatch):
    monkeypatch.setenv("ORCHE_MAX_COST_USD", "2.50")
    cost.record(0.25)
    text = cost.status()
